...
/**
 * Create and sign a JWT payment request.
 *
...
pay.request = function(request) {
  _requireConfig();
  return jwt.sign(pay.issueRequest(request), config.mozPaySecret,
                  {algorithm: 'HS256'});
};
...
pay.verify = function(foreignJwt) {
  _requireConfig();
  return jwt.verify(foreignJwt, config.mozPaySecret,
...
pay.routes = function(app, options) {
  ...
  function handle(req, res, onData) {
    ...
    try {
      data = jwt.verify(notice, config.mozPaySecret);
    } catch (er) {
